  ...
  ({ strapi }) => ({
    ...
    async create(ctx) {
      ...
      try {
        ...
        const existingReaction = await strapi.entityService.findMany(
          'api::reaction.reaction',
          {
            filters: {
        ...
        if (existingReaction.length > 0) {
          ...
          const updatedReaction = await strapi.entityService.update(
            'api::reaction.reaction',
            reactionId,
            {
              data: {
          ...
          const reaction = await strapi.entityService.create(
            'api::reaction.reaction',
            {
              data: {
                emoji,
          ...
          const { reaction_count } = await strapi.entityService.findOne(
            'api::project.project',
            project,
          );
    ...
    async delete(ctx) {
      ...
      const reaction = await strapi.entityService.findMany(
        'api::reaction.reaction',
        {
          filters: {
            project: id,
